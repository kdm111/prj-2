...
| W4 | 7/28–8/3 | **마커리스(HSV 색검출 + RealSense 깊이) 우선** → 물체 위치 → `/scene_state`. **ArUco는 폴백**(레포에 `camera_realsense.launch.py`·`camera_usb_cam.launch.py` 존재). 딥러닝 6D는 W8 카드. agent 실스킬 연결. G4, G8 | "빨간 블록 트레이에" 데모 |
...
## 12. W2 진행 로그 — 해석적 IK (2026-07-22~)

### 12.1 축·기하 확인 — 파일 검증 완료
- URDF `<axis>` (`omx_f_arm.urdf.xacro`): **j1=z, j2·j3·j4=y, j5=x**. §2.4.2 IK 서사 전제 **성립 확정**.
- 체인 오프셋: j1(-0.01125,0,0.034) / j2(0,0,0.0635) / j3(0.0415,0,0.11315) / j4(0.162,0,0) / j5(0.0287,0,0).
- **TCP:** `end_effector_joint`(fixed) link5→ee = **(0.09193, −0.0016, 0)**. IK 대상 = `end_effector_link` (SRDF, KDL과 동일 → 벤치 공정). TCP가 롤축(x)에서 y로 −1.6mm만 벗어남 → **j5는 거의 위치-무관**.

### 12.2 분해 전략 — 확정
- **모터1(겨눔):** 3D→2D. `θ1 = atan2(py, px)`. (베이스 x-오프셋 11.25mm·TCP y −1.6mm는 현재 무시, 30cm 목표서 2~3° 이내. 필요시 보정.)
- **모터2·3(도달):** 평면 2R, 코사인법칙. 팔꿈치 위/아래 **2해**.
- **모터4(방향):** 남는 1 DOF = 접근 방향. 피킹=그리퍼 아래. **실제 계산 순서: 방향 먼저 정 → wrist point 산출 → 2·3이 도달 → 4가 방향 마무리.**
- **모터5(롤):** 위치 거의 무관, 파지 방향용.

### 12.3 유도 진척 — 개념·수식 5모터 전부 ✅ (종이 완료, 숫자 대입만 남음)
- ✅ `θ1 = atan2(py, px)` (겨눔)
- ✅ wrist point: 방향 먼저 정함(피킹=그리퍼 아래) → `wrist = 목표 − L3·(접근방향)`. 아래수직이면 `(r, z+L3)`.
- ✅ 팔꿈치 `θ3`: `cos(θ_elbow) = (L1²+L2²−D²)/(2·L1·L2)`, `D=√(r²+z²)` (단 여기 r·z는 **wrist point** 기준). **주의:** 분모 전체 괄호. **D=변/θ=각** 구분.
- ✅ 어깨 `θ2`: `atan2(z,r) ± β`, `cos(β)=(L1²+D²−L2²)/(2·L1·D)`. ±는 팔꿈치와 **짝**(4해 아님, 위/아래 **2해**).
- ✅ `θ4 = φ − θ2 − θ3` (2·3·4 평행축 → 각이 더해짐, φ=접근각).
- ✅ `θ5 = 0` 잠정 (롤, 위치 무관. 인지 붙으면 물체 yaw로 채움).
- **★ UNREACHABLE 판정 여기서 태어남:** `|cos| > 1` (D>L1+L2 또는 D<|L1−L2|, 도넛 밖) → 해 없음 → `ErrorCode.UNREACHABLE`(§4.3, ABORT). **코드: `acos` 전 `if(abs(cos)>1)` 필수(안 하면 NaN).**
- 남음: **실제 L1·L2·L3 대입**(오프셋이 대각선 → 링크에 고정각 박힘, 순수 길이 아님) → **C++ 라이브러리 + gtest FK 왕복** → KDL 대비 성공률·시간 벤치(G6).
